...
# ---------------------------
# Holiday Features (KR 공휴일)
# ---------------------------
def build_holiday_dates(train_df, test_df):
    start = min(train_df['일시'].min(), test_df['일시'].min()).date()
    end   = max(train_df['일시'].max(), test_df['일시'].max()).date()
    ...
    try:
        ...
        # Fallback: 고정일 + 간단 대체휴일(일요일과 겹치면 다음날)
...
def add_holiday_feats(df, holiday_dates):
    ...
    hol = pd.to_datetime(list(holiday_dates))
...
holiday_dates = build_holiday_dates(train, test)
train = add_holiday_feats(train, holiday_dates)
test  = add_holiday_feats(test, holiday_dates)
...
def objective(trial):
    ...
    for tcode in train['건물유형'].unique():
        ...
        for (tr_mask, va_mask, _rng) in folds:
            ...
            xgb_model = xgb.train(
                xgb_fixed, dtr, num_boost_round=10000,
                evals=[(dva, 'valid')], early_stopping_rounds=200, verbose_eval=False
            )
...
for tcode in tqdm(sorted(train['건물유형'].unique()), desc="Final train/predict by type"):
    ...
    # Blend → inverse log → clip → 기준선 스무딩
