...
class QuantumCircuit(Module):
    def __init__(self):
        super().__init__()
        # 2 qubits, 4 layers -> 2*4*3=24 trainable params
        self.weights = torch.nn.Parameter(torch.randn(24, dtype=torch.float64))
        
        @qml.qnode(dev, interface="torch", diff_method="adjoint")
        def circuit(inputs, weights):
            qml.AngleEmbedding(inputs, wires=[0,1])
            qml.StronglyEntanglingLayers(weights.reshape(4,2,3), wires=[0,1])
...
class HybridCNN(Module):
    def __init__(self, activation):
        ...
        # FC 레이어 노드 수 확장
        self.fc1   = Linear(48,   128) # Global Average Pooling 후 입력 크기: 48
        self.fc2   = Linear(128,  12)
    ...
    def forward(self, x):
        ...
        q_out = self.qc(qnn_input, self.qc.weights)
