...
from tensorflow.keras.layers import Dense, LSTM, Flatten, Dropout, BatchNormalization, MaxPooling2D
import time
...
model = Sequential()
model.add(LSTM(512,  input_shape=(32*32,3), activation='relu'))
...
start = time.time()
hist = model.fit(x_train, y_train, epochs=2, batch_size=256, # batch는 행이다!!!!!!!!
                 verbose=1,
