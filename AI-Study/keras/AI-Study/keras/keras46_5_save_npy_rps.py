...
xy_train = train_datagen.flow_from_directory(
    path_train,                    #경로
    target_size=(100, 100),        #원 사이즈 150,150 가 해당 사이즈로 자동변환 됨 resize
    batch_size=100,                #메모리 사이즈 때문에 batch를 통배치로 하지 않는다
    class_mode='categorical',           #다중분류 / 원핫 자동으로 됨
    # class_mode='sparse'               #다중분류 / 원핫 x 모델구성할 때 원핫하면 됨
    # class_mode='binary'               #이진분류
    # class_mode=None                   #y값이 없이 만들어짐
    color_mode='rgb',        #색깔
    shuffle=True,
    seed=333,
)
