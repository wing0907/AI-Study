...
model = Sequential()
model.add(Conv2D(5120, (3,3), strides=1, input_shape=(32,32,3), activation='relu'))
model.add(MaxPooling2D())      
model.add(BatchNormalization())
model.add(Dropout(0.3))
model.add(Conv2D(filters=5120, kernel_size=(3,3), activation='relu'))
model.add(MaxPooling2D())                                            
model.add(BatchNormalization())
model.add(Dropout(0.3))
...
model.add(Dense(units=12800, activation='relu'))
model.add(BatchNormalization())
model.add(Dropout(0.2))
...
path = './_save/keras39/'
filename = '.hdf5'
filepath = "".join([path, 'k39_04',filename])
#######################################################
mcp = ModelCheckpoint(
    monitor='val_loss',
    mode='auto',
...
start = time.time()
hist = model.fit(x_train, y_train, epochs=100000, batch_size=50000, # batch는 행이다!!!!!!!!
                 verbose=1,
...
# loss :  2.4016194343566895
# acc :  0.4198000133037567
# 걸린시간: 1128.7073063850403 초
