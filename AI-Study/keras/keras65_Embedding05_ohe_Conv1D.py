...
# 4. get_dummies → 원핫 인코딩 (15, 5, vocab_size)
vocab_size = len(token.word_index) + 1
x_oh = []
for seq in x_pad:
    temp = pd.get_dummies(seq).reindex(columns=range(vocab_size), fill_value=0)
    x_oh.append(temp.values)
x_oh = np.array(x_oh)  # shape: (15, 5, vocab_size)

# 5. train/test 분할
x_train, x_test, y_train, y_test = train_test_split(x_oh, y, test_size=0.3, random_state=222)

# 6. Conv1D + Flatten 모델 구성 (❗Embedding 제거!)
model = Sequential()
...
# loss: 0.877758800983429 acc: 0.4000000059604645
# [이삭이 참 잘생겼다]의 결과 : [[0.6589107]]
# >> 예측 결과: 긍정 (확률 0.66)