# TF1-style MLP on Wine (ReLU + Dropout, 3 classes)
import numpy as np
...
num_classes = 3
y = np.eye(num_classes, dtype=np.float32)[y_int]  # 원-핫 (178, 3)
...
# 5) MLP: 13 -> 64 -> 32 -> 3 (은닉 ReLU + Dropout, 출력은 로짓)
w1 = tf.compat.v1.Variable(he(shape=[n_features, 64])); b1 = tf.compat.v1.Variable(tf.zeros([64]))
h1 = tf.nn.relu(tf.matmul(x, w1) + b1)
h1 = tf.nn.dropout(h1, keep_prob=keep_prob1)   # ▶ 훈련 시만 0.9 등으로 적용

w2 = tf.compat.v1.Variable(he(shape=[64, 32])); b2 = tf.compat.v1.Variable(tf.zeros([32]))
h2 = tf.nn.relu(tf.matmul(h1, w2) + b2)
h2 = tf.nn.dropout(h2, keep_prob=keep_prob2)

w3 = tf.compat.v1.Variable(he(shape=[32, num_classes])); b3 = tf.compat.v1.Variable(tf.zeros([num_classes]))
