...
def load_dataset(data_dir):
    # train.csv 우선, 없으면 diabetes.csv, 없으면 폴더 내 유일 CSV
    for name in ["train.csv", "diabetes.csv"]:
...
def guess_target_column(df):
    ...
    for c in candidates:
        if c in df.columns:
            return c
    # 없다면 이진(고유값 2개) 컬럼을 뒤에서 앞으로 탐색
    for c in df.columns[::-1]:
        if df[c].nunique(dropna=True) == 2:
            return c
    raise ValueError("타깃 컬럼을 찾을 수 없습니다. (Outcome/target/label/y 등 확인)")
...
print("\n=== Dacon Diabetes (ReLU + Dropout MLP) ===")
...
# === Dacon Diabetes (ReLU + Dropout MLP) ===
# Train Acc: 0.9981
#  Test Acc: 0.7252 | ROC-AUC: 0.7864
