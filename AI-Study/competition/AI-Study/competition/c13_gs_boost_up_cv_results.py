...
# 성능 기록을 위한 리스트
best_model = None
best_val_loss = float('inf')  # 최소값으로 초기화
...
for fold, (train_idx, val_idx) in enumerate(kf.split(X_scaled, y_log)):
    ...
    if val_loss < best_val_loss:
        ...
        best_model = model  # 현재 모델을 저장

# 모델 저장 (최고 성능 모델)
best_model_path = path + f'best_dnn_model_{now}.h5'
best_model.save(best_model_path)
print(f"Best DNN 모델 저장 완료: {best_model_path}")

# 예측 및 평가 (최고 성능 모델 사용)
pred_train_log = best_model.predict(X_train)
...
# Train R² Score: -0.1298, RMSE: 28.2590, MAE: 22.4872
# Test  R² Score: -0.0993, RMSE: 26.8669, MAE: 20.8836
# 4/4 [==============================] - 0s 3ms/step
# 제출 파일 저장 완료: C:/Study25/_data/dacon/boost_up/submission_dnn_20250716_121948.csv