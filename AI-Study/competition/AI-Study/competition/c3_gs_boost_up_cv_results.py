...
# 모델 저장
joblib.dump(final_model, path + 'c3_model_0708_02.pkl')
print("모델 가중치 저장 완료: c3_model_0708_02.pkl")
...
# -----------------------------
# 7. 평가 지표 계산 (Normalized RMSE, Correlation, Score)
# -----------------------------
max_inhibition = 99.3815
min_inhibition = 0.0
range_inhibition = max_inhibition - min_inhibition

normalized_rmse = rmse / range_inhibition
correlation = np.corrcoef(y_val, y_val_pred)[0, 1]
score = 0.5 * (1 - min(normalized_rmse, 1)) + 0.5 * correlation

print(f"\nMax Inhibition: {max_inhibition:.4f}")
print(f"Min Inhibition: {min_inhibition:.4f}")
print(f"선형 상관계수: {correlation:.4f}")
print(f"Normalized RMSE: {normalized_rmse:.4f}")
print(f"최종 Score: {score:.4f}")
...
submission.to_csv(path + 'c3_submission_0708_02.csv', index=False)
print("제출 파일 저장 완료: c3_submission_0708_02.csv")

# 모델 가중치 저장 완료: c3_model_0708_01.pkl
# RMSE: 5.2347
# R2 Score: 0.9585
# SEED : 222
...
# 선형 상관계수: 0.9861
# Normalized RMSE: 0.0527
# 최종 Score: 0.9667
# 제출 파일 저장 완료: c3_submission_0708_01.csv
